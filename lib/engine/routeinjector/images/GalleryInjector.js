...
var GalleryInjector = /** @class */ (function () {
    ...
    GalleryInjector.prototype.handleGetImage = function () {
        ...
        this.routeInjector.app.use(function (req, res, next) { return __awaiter(_this, void 0, void 0, function () {
            ...
            var fileName, fileAbs, size, _a, widthStr, heightStr, width, height, fileNameNoExt, outputFile, err_1;
            return __generator(this, function (_b) {
                switch (_b.label) {
                    case 0:
                        _b.trys.push([0, 9, , 10]);
                        if (!req.url.startsWith(this.prefix + this.galleryEndpoint)) return [3 /*break*/, 7];
                        fileName = req.url.split("/").pop();
                        fileAbs = path.join(this.routeInjector.config.env.images.path, fileName);
                        return [4 /*yield*/, util_2.promisify(fs.exists)(fileAbs)];
                    case 1:
                        if (!_b.sent()) return [3 /*break*/, 5];
                        if (!supportsWebP(req.headers)) return [3 /*break*/, 3];
                        size = req.url.split("/").slice(-2)[0];
                        _a = size.split("x"), widthStr = _a[0], heightStr = _a[1];
                        width = !isNaN(parseInt(widthStr)) ? parseInt(widthStr) : null;
                        height = !isNaN(parseInt(heightStr)) ? parseInt(heightStr) : null;
                        fileNameNoExt = fileName.split(".").slice(0, -1).join(".");
                        outputFile = path.join(this.routeInjector.config.env.images.cache, width ? size : "", fileNameNoExt + ".webp");
