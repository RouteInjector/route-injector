"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : new P(function (resolve) { resolve(result.value); }).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
var _this = this;
var HooksUtils = /** @class */ (function () {
    function HooksUtils() {
    }
    HooksUtils.funcMerge = function (funcArray) {
        return function (doc, cb, state) { return __awaiter(_this, void 0, void 0, function () {
            var result, _i, funcArray_1, func;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        result = doc;
                        _i = 0, funcArray_1 = funcArray;
                        _a.label = 1;
                    case 1:
                        if (!(_i < funcArray_1.length)) return [3 /*break*/, 4];
                        func = funcArray_1[_i];
                        return [4 /*yield*/, new Promise(function (resolve) {
                                func(result, resolve, state);
                            })];
                    case 2:
                        result = _a.sent();
                        if (!result)
                            return [2 /*return*/, cb(result)];
                        _a.label = 3;
                    case 3:
                        _i++;
                        return [3 /*break*/, 1];
                    case 4: return [2 /*return*/, cb(result)];
                }
            });
        }); };
    };
    HooksUtils.iterateCursor = function (cursor, limit, func, ctx) { return __awaiter(_this, void 0, void 0, function () {
        var partial, state, doc_1, next, _a, result, _b, err_1;
        return __generator(this, function (_c) {
            switch (_c.label) {
                case 0:
                    partial = [];
                    state = ctx;
                    _c.label = 1;
                case 1:
                    _c.trys.push([1, 15, , 16]);
                    return [4 /*yield*/, cursor.next()];
                case 2:
                    doc_1 = _c.sent(), next = void 0;
                    if (!doc_1) return [3 /*break*/, 6];
                    if (!(limit > 1)) return [3 /*break*/, 4];
                    return [4 /*yield*/, cursor.next()];
                case 3:
                    _a = _c.sent();
                    return [3 /*break*/, 5];
                case 4:
                    _a = null;
                    _c.label = 5;
                case 5:
                    next = _a;
                    _c.label = 6;
                case 6:
                    state.ctx.hasNext = !!next;
                    _c.label = 7;
                case 7:
                    if (!doc_1) return [3 /*break*/, 14];
                    if (!(limit > 0)) return [3 /*break*/, 12];
                    return [4 /*yield*/, new Promise(function (resolve) {
                            func(doc_1, resolve, state);
                        })];
                case 8:
                    result = _c.sent();
                    if (result) {
                        limit--;
                        partial.push(result);
                    }
                    doc_1 = next;
                    if (!(limit > 1)) return [3 /*break*/, 10];
                    return [4 /*yield*/, cursor.next()];
                case 9:
                    _b = _c.sent();
                    return [3 /*break*/, 11];
                case 10:
                    _b = null;
                    _c.label = 11;
                case 11:
                    next = _b;
                    state.ctx.hasNext = !!next;
                    return [3 /*break*/, 13];
                case 12: return [2 /*return*/, state.result || partial];
                case 13: return [3 /*break*/, 7];
                case 14: return [2 /*return*/, state.result || partial];
                case 15:
                    err_1 = _c.sent();
                    return [2 /*return*/, state.result || partial];
                case 16: return [2 /*return*/];
            }
        });
    }); };
    return HooksUtils;
}());
module.exports = HooksUtils;
//# sourceMappingURL=HooksUtils.js.map